...
any doEval(any x) {
   ...
   if (!isNum(y = EVAL(car(x))) || !(p = Env.bind))
      ...
      struct {  // bindFrame
         ...
         int i, cnt;
         struct {any sym; any val;} *bnd;
      } f;

      f.bnd = alloca(length(x) * 2 * sizeof(any));
...
any doBind(any ex) {
   ...
   }
   {
      ...
      f.bnd = alloca(length(y) * 2 * sizeof(any));
      f.link = Env.bind,  Env.bind = (bindFrame*)&f;
      f.i = f.cnt = 0;
      do {
         if (isNum(car(y)))
...
any doLet(any x) {
   ...
   if (!isCell(y = car(x))) {
      ...
      f.bnd = alloca(((length(y)+1)/2) * 2 * sizeof(any));
      f.link = Env.bind,  Env.bind = (bindFrame*)&f;
      f.i = f.cnt = 0;
      do {
         f.bnd[f.cnt].sym = car(y);
